...
async function fetchConduits() {
  // before starting the gateway so we have data to test...
...
if (!module.parent) {
  ...
  // if we can't login then there's no point in running the gateway
  app.listen(conf.gwServerPort, 'localhost', () =>
    console.log(
      `Conduits gateway server is listening on port ${conf.gwServerPort}`
    )
  );
}
