...
[`examples/`](../examples/README.md)). Buildless HTML/CSS/JS, no
...
stable `/library/` URL prefix that predates this directory's own
`packages/widgets` name, so every widget below is a real, linkable
URL — e.g. `https://conduits.xyz/library/xyz-waitlist/`.

Every widget uses the `xyz-` custom-element prefix, and every one of
them — along with `examples/basic-form` and `examples/basic-ajax-form`
...
See [`examples/widget-gallery/`](../examples/widget-gallery/README.md)
to try every widget here side by side against a real conduit URL,
