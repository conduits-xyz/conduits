...
;(function () {
  ...
  class XyzReactions extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._votedReaction = this._readStoredVote()
      ...
      if (!this.hasAttribute('initial-up') && !this.hasAttribute('initial-down') && this.conduitUrl) {
        this._refreshCounts()
      }
    }
    ...
    async _vote(reaction) {
      if (this._pending || this._votedReaction) return
      this._pending = true
      this._error = null
    ...
    _render() {
      ...
      const disabled = this._pending || Boolean(voted)
      ...
      this.innerHTML = `
        <article class="xyz-reactions-widget">
          ...
          <p class="xyz-reactions-status" data-state="${this._error ? 'error' : 'success'}" role="status" aria-live="polite">${
            this._error ? escapeHtml(this._error) : voted ? escapeHtml(this.successMessage) : ''
          }</p>
        </article>
      `
