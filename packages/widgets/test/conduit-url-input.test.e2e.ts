...
// Exactly 12 characters — conduit-url-input.js's own CURI_PATTERN only
// ever treats a bare value this shape as "just a curi," never a
...
describe('conduit-url-input.js — bare curi resolved against the current origin (e2e)', () => {
  it('shows this origin as the prefix, resolves a bare curi against it, and completes a real signup', async (t) => {
    ...
    await page.getByLabel('Conduit URL').blur()
  ...
  it('rejects a value that is not a valid curi shape, still against a known (non-file://) origin', async (t) => {
    process.env.WIDGET_E2E_FASTMAIL_TOKEN = 'widget-e2e-fastmail-token'
    ...
    await page.goto(server.baseUrl + '/xyz-waitlist/')
    await page.getByLabel('Conduit URL').fill('not-twelve-chars-long')
    await page.getByLabel('Conduit URL').blur()
    // "Enter a valid conduit ID." (not "...URL.") is only ever shown
    // once a prefix is known — see setupConduitUrlInput's own check().
    // Confirms this rejection took the bare-curi branch, not the
    // no-known-origin one.
    await page.getByText('Enter a valid conduit ID.').waitFor()
  })
})
